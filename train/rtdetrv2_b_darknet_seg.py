...
from haua.models import RTDETRv2InSeg
from haua.criteriones import RTDETRInstanceCriterionv2, HungarianMatcher
...
class TrainRTDETRv2InSeg(BaseModel):
    """MMEngine 封装的多任务模型"""
    def __init__(
        self,
        model_config: dict,
        ...
        self.matcher = HungarianMatcher(
            ...
            alpha = 0.25,
            gamma = 2.0)
        self.model = RTDETRv2InSeg(**model_config)
        if init_checkpoint is not None:
            init_static_dict = torch.load(init_checkpoint)
            self.model.load_state_dict(init_static_dict)
        self.loss_module = RTDETRInstanceCriterionv2(self.matcher, **loss_config)
...
if __name__ == '__main__':
    ...
    MODELS.register_module(module=TrainRTDETRv2InSeg)
